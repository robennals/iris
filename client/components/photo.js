...
import { FixedTouchable, GroupIcon, makePhotoDataUrl, MemberIcon, memberKeysToHues } from './basics';
...
export function MemberProfilePhotoPlaceholder({size = 150}) {
    return (
        <View style={{justifyContent: 'space-around', alignItems: 'center', width: size, height: size, borderRadius: size/2, borderColor: '#ddd', borderWidth: StyleSheet.hairlineWidth}}>
...
export function MemberPhotoIcon({photoKey, hue, user, name, style, size = 40, thumb=true}) {
  if (user && photoKey) {
        return (
      ...
      return <MemberIcon name={name} hue={hue} style={style} size={size} />
  }
}

function MiniMemberPhoto({members, hue, user, style, size}) {
  if (user) {
    return <MemberPhotoIcon hue={hue} photoKey={members[user].photo} user={user} name={members[user].name} style={style} size={size} />
  } else {
    return null;
  }
}
...
export function GroupSideBySideIcon({members, size}) {
  ...
  return (
    <View style={{flexDirection: 'row', marginLeft: size/8}}>
      {keys.map(k => 
        <MiniMemberPhoto hue={memberHues[k]} key={k} members={members} user={k} size={size} style={{marginLeft: -size/4}} />
      )}
    </View>
  )
}
